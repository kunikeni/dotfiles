...
## Chezmoi-managed repos (CRITICAL)

Before editing anything, check whether the current repo is chezmoi's source directory (`.chezmoitemplates/` and `dot_claude/` present at the repo root). If it is:

- **Never edit the real files under `~/.claude/`** (`~/.claude/rules/*.md`, `~/.claude/CLAUDE.md`, `~/.claude/skills/*/SKILL.md`, etc.). They are generated by `chezmoi apply`; a direct edit there is either overwritten on the next apply or silently diverges from source — this is the exact failure this skill exists to prevent.
...
- Run `chezmoi diff` before reporting completion. The diff must show only the intended change — if it also shows unrelated content already present in the real file but missing from source, that is pre-existing drift from a past direct edit; report it to the user instead of silently ignoring or silently folding it in.

Outside a chezmoi repo, edit `~/.claude/rules/*.md` or the project's `CLAUDE.md` directly — there is no source/real-file split to worry about.
